...
Процедура СохранитьИзменения(Команда)
	...
	Если Элементы.ДеревоДокументов.ТекущаяСтрока <> Неопределено Тогда
		...
		Если ДанныеСтрокиЗаполненыКорректно(Элементы.ДеревоДокументов.ТекущиеДанные) Тогда
...
Процедура ОткрытьПеремещение(Команда)
	...
	ТекущаяСтрока = Элементы.ДеревоДокументов.ТекущаяСтрока;
	
	Если ТекущаяСтрока = Неопределено Тогда
		ПоказатьПредупреждение(, "Выберите строку выписки!");
	Иначе
		
		ТекущиеДанные = ДеревоДокументов.НайтиПоИдентификатору(ТекущаяСтрока);
		
		Если ЗначениеЗаполнено(ТекущиеДанные.Документ) И 
				Не РазрешеноОткрытиеПеремещения(ТекущиеДанные.Документ) Тогда
			
			ПоказатьПредупреждение(, "Недопустимая операция для этого вида документа!");
		
		Иначе
				 
			ОткрытьФормуПеремещения(ТекущиеДанные);
...
Процедура ОбновитьНаСервере()
	...
	ВалютаСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Счет, "Валюта");
	...
	|	БанковскаяТранзакция.СуммаВВалютеСчета,
	|	БанковскаяТранзакция.ПометкаУдаления,
	...
	|	Транзакции.СуммаВВалютеСчета Как СуммаВВалютеСчета,
	|	1 Как Курс,
	|	1 Как Кратность,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.СуммаВВалютеСчета) Тогда
			
			Ветвь.Сумма = Выборка.СуммаВВалютеСчета;
			Ветвь.Валюта = ВалютаСчета;
...
Процедура СохранитьДанныеДокумента(ТекущаяСтрокаДерева)
	...
	СоздаватьОбновлятьДокумент = ТекущиеДанные.Проведен 
						Или Не ТекущиеДанные.Проведен И Не ЗначениеЗаполнено(ТекущиеДанные.Документ);
	
	Если СоздаватьОбновлятьДокумент Тогда
...
Процедура ПеренестиДанныеИзШаблонаВПлатеж(СтрокаДанных, СтрокаШаблона)
	...
	Если СтрокаДанных.Документ = Неопределено И Не ПустаяСтрока(СтрокаШаблона.Комментарий) Тогда
...
&НаКлиенте
Функция РазрешеноОткрытиеПеремещения(Документ)
	
	ЗапрещенныеТипы = Новый Массив;
	ЗапрещенныеТипы.Добавить(Тип("ДокументСсылка.ПриходДенежныхСредств"));
	ЗапрещенныеТипы.Добавить(Тип("ДокументСсылка.РасходДенежныхСредств"));
	
	Возврат ЗапрещенныеТипы.Найти(ТипЗнч(Документ)) = Неопределено;
	
КонецФункции

&НаКлиенте
Процедура ОткрытьФормуПеремещения(ДанныеСтроки)
	
	ДанныеЗаполнения = Новый Структура();
	ДанныеЗаполнения.Вставить("ВидДвижения", 	ДанныеСтроки.ВидДвижения);
	ДанныеЗаполнения.Вставить("Счет", 			Счет);
	ДанныеЗаполнения.Вставить("Сумма", 			ДанныеСтроки.Сумма);
	ДанныеЗаполнения.Вставить("Дата", 			ДанныеСтроки.ДатаТранзакции);
	ДанныеЗаполнения.Вставить("Валюта", 		ДанныеСтроки.Валюта);
	ДанныеЗаполнения.Вставить("Курс", 			ДанныеСтроки.Курс);
	ДанныеЗаполнения.Вставить("Кратность", 		ДанныеСтроки.Кратность);
	ДанныеЗаполнения.Вставить("Комментарий", 	ДанныеСтроки.СодержаниеТранзакции);
	ДанныеЗаполнения.Вставить("Транзакция", 	ДанныеСтроки.БанковскаяТранзакция);
		
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Ключ", 		ДанныеСтроки.Документ);
	...
	ООЗакрытиеПеремещения = Новый ОписаниеОповещения("ЗакрытиеФормыПеремещения", ЭтотОбъект, 
														ДополнительныеПараметры);
	
	ОткрытьФорму("Документ.ПеремещениеДенежныхСредств.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект, , , , 
					ООЗакрытиеПеремещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца)
	
КонецПроцедуры

&НаКлиенте
Процедура ЗакрытиеФормыПеремещения(Результат, ДополнительныеПараметры) Экспорт
	
	ДанныеСтроки = ДополнительныеПараметры.ДанныеСтроки;
	
	Если ЗначениеЗаполнено(ДанныеСтроки.Документ) Тогда
		ДанныеСтроки.Проведен = Истина;
	КонецЕсли;
