...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьСчета();
	Элементы.Счет.КнопкаСоздания = Ложь;
...
Процедура ЗаполнитьСчета()
	...
	|ИЗ
	|	Справочник.Счета КАК Счета
	|	Где Счета.ФорматВыписки <> ЗНАЧЕНИЕ(Перечисление.ФорматыВыписки.ПустаяСсылка)";
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Элементы.Счет.СписокВыбора.Добавить(Выборка.Ссылка, Выборка.Представление);
	КонецЦикла;	
	
	Элементы.Счет.РежимВыбораИзСписка = Истина;
