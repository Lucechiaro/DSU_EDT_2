...
Процедура ДобавитьСтрокуТранзакции(ТаблицаТранзакций, МассивПоказателей)
	...
	СуммаСтрокой = МассивПоказателей[4];	
	ЗаменитьСпециальныеСимволы(СуммаСтрокой);
	Сумма = Число(СуммаСтрокой);
	
	СуммаВВалютеСчетаСтрокой = МассивПоказателей[6];	
	ЗаменитьСпециальныеСимволы(СуммаВВалютеСчетаСтрокой);
	СуммаВВалютеСчета = Число(СуммаВВалютеСчетаСтрокой);
...
Процедура ЗаменитьСпециальныеСимволы(СуммаСтрокой)
	
	СпециальныеСимволы = Новый Соответствие;
	СпециальныеСимволы.Вставить("""", "");
	СпециальныеСимволы.Вставить(" ", "");
	
	Для Каждого СпециальныйСимвол Из СпециальныеСимволы Цикл
		
		СуммаСтрокой = СтрЗаменить(СуммаСтрокой, СпециальныйСимвол.Ключ, СпециальныйСимвол.Значение);
