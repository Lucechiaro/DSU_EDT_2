...
Процедура СписокДокументовПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяСтрока <> Элемент.ТекущаяСтрока Тогда
		...
		Если МодифицированностьДанных Тогда 
			...
			Ответ = Вопрос("Имеются несохраненные данные! Записать документ?", РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ЗаписатьДокументНаСервере(ДанныеФормы, Ссылка, РежимЗаписи)
	...
	//Объект = ДанныеФормыВЗначение(ДанныеФормы, Тип("ДокументОбъект.РасходДенежныхСредств"));
	...
	Если Ссылка.Пустая() Тогда
		...
		//Исключение
		//	Сообщить("Не удалось записать документ!" + Символы.ПС + "Проверьте, заполнены ли все необходимые реквизиты");
		//КонецПопытки;
		
	Иначе  
		
		//// перезаполняем объект документа из ссылки на основе данных формы
		//ОбъектИзСсылки = ДокументСсылка.ПолучитьОбъект();
		//
		//Если МодифицированностьДанных Тогда
		//	ЗаполнитьЗначенияСвойств(ОбъектИзСсылки, Объект, "Дата, Счет,СтатьяДДС,Валюта,Сумма,Курс,Кратность,ПланируемаяПокупка,Должник,НеУчитыватьВСтруктуреРасходов,Комментарий");
		//КонецЕсли;
		//ОбъектИзСсылки.Записать(РежимЗаписиНовый);
		Объект.Записать(РежимЗаписиНовый);
...
Процедура ПрочитатьДокументНаСервере()
	...
	//ЗаполнитьЗначенияСвойств(Документ, ДокументСсылка);
